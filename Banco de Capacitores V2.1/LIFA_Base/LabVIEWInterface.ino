...
{  
  ...
  //if(command[0] == 0xFF)
  {
    ...
    *********************************************************************************/
    ...
    /*********************************************************************************
    ** LEITURA DE CAPACITANCIA
    *********************************************************************************/        
      case 0x35:  //LEITURA DE CAPACITANCIA //COMMAND 2 PINO ANALOGICO; COMMAND 3 SAIDA DO RESISTOR
      Erro="";
      //#####################DESCARREGA CAPACITOR############################# 
      
      digitalWrite(command[3], LOW); //DESLIGA RESISTOR DE LEITURA

      t0 = micros();
      t1=0;
      /*
      contando=true;
      while(contando && (t1<2000000)){
      retVal = analogRead(command[2]);
              if (retVal<=10){   
                contando = false;       //SAIR DO LOOP
                  if(digitalRead(11)==LOW){
                  digitalWrite(11, HIGH); //DESLIGA RESISTOR DE DESCARGA
                  delay(10);
                  }
              }
              else
              {
                digitalWrite(11, LOW); //LIGA RESISTOR DE DESCARGA
              }
        t1 = micros()-t0;  // calculate elapsed time  
      }
*/
   //#########################FIM DESCARREGA CAPACITOR#####################################
    
  if(t1<2000000){
      
      //#########################LE CAPACITOR#################################################
            contando= true;
            digitalWrite(command[3], HIGH); // LIGA RESISTOR DE LEITURA
            t0 = micros();
            t1=0;
            while(contando && (t1<2000000)){
              retVal = analogRead(command[2]);
                      if (retVal>= 647){   //647= 63.2% of 1024 !! (capacitor "Tau" constant)
                        contando = false;
                      }
              t1 = micros()-t0;  // calculate elapsed time          
            }
	...
	Erro="Er02";   // ERRO AO DESCARREGAR NO INICIO
  }
//#####################DESCARREGA CAPACITOR############################# 
      tempo=t1;
      t0 = micros();
      t1=0;
      contando=true;
      while(contando==true && (t1<2000000)){
      retVal = analogRead(command[2]);
              if (retVal<=10){   
                contando = false;       //SAIR DO LOOP
                if(digitalRead(11)==LOW){
    ...
    if (t1<2000000){
    if(tempo<2000000){
      Serial.write ((char *)&tempo,4);
    }
    else{
      Erro="Er01";  //ERRO AO LER
      }
    }else{
      Erro="Er03"; //ERRO AO DESCARREGAR NO FINAL
    }
    if(Erro!=""){
      Serial.print(Erro);
    }
